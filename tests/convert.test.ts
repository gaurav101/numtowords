...
describe("English (en)", () => {
  ...
  it("converts 999", () => expect(convert(999)).toBe("Nine hundred and ninety-nine"));
  ...
  it("converts one million", () => expect(convert(1_000_000)).toBe("One million"));
  it("converts 1 billion", () => expect(convert(1_000_000_000)).toBe("One billion"));
  ...
  it("accepts bigint input", () => expect(convert(9007199254740993n)).toContain("quadrillion"));
  it("disables 'and'", () =>
...
describe("Indian system (in)", () => {
  ...
  it("1,000,124 = ten lakh one hundred twenty-four", () =>
    expect(convert(1_000_124, { locale: "in" })).toBe("Ten lakh one hundred twenty-four"));
  ...
  it("100 crore = one arab", () =>
    expect(convert(1_000_000_324n, { locale: "in" })).toBe("One arab three hundred twenty-four"));
});
...
describe("Currency option", () => {
  ...
  it("appends currency to Indian", () =>
    expect(convert(100_000, { locale: "in", currency: "INR" })).toBe("One lakh INR"));
});
